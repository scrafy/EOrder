...
class ChangePassword
{
    ...
    @RegexFormatValidation("The old password must containt at least one digit, one lower case, one special character and a lenght of 6 characters as minimun", "oldPassword","^(?=.*[0-9]+.*)(?=.*[a-z]+.*)(?=.*[A-Z]*.*)(?=.*\\W+.*)[0-9a-zA-Z\\W]{6,}\$",2)
    var oldPassword: String? = null

    @NullOrEmptyStringValidation("The new password field can not be empty or null", "newPassword",1)
    @RegexFormatValidation("The new password must containt at least one digit, one lower case, one special character and a lenght of 6 characters as minimun", "newPassword","^(?=.*[0-9]+.*)(?=.*[a-z]+.*)(?=.*[A-Z]*.*)(?=.*\\W+.*)[0-9a-zA-Z\\W]{6,}\$",2)
